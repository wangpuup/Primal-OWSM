...
from espnet2.s2t_nystrom.ctc import CTC
...
class ESPnetS2TPRIMALModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 1. Encoder
        encoder_out, encoder_out_lens, ksvd_loss = self.encode(speech, speech_lengths)
        ...
        if self.ctc_weight == 0.0:
            loss = loss_att + self.ksvd_weight * ksvd_loss
        elif self.ctc_weight == 1.0:
            loss = loss_ctc + self.ksvd_weight * ksvd_loss
        else:
            loss = self.ctc_weight * loss_ctc + (1 - self.ctc_weight) * loss_att + self.ksvd_weight * ksvd_loss
                
        # Collect Attn branch stats
        stats["loss_att"] = loss_att.detach() if loss_att is not None else None
        stats["loss_ksvd"] = ksvd_loss.detach() if ksvd_loss is not None else None
        stats["acc"] = acc_att
        stats["cer"] = cer_att
        stats["wer"] = wer_att
    ...
    def encode(
        ...
        if self.encoder.interctc_use_conditioning:
            encoder_out, encoder_out_lens, ksvd_loss, _ = self.encoder(
                feats, feats_lengths, ctc=self.ctc
            )
        else:
            encoder_out, encoder_out_lens, ksvd_loss, _ = self.encoder(feats, feats_lengths)
        ...
        if intermediate_outs is not None:
            return (encoder_out, intermediate_outs), encoder_out_lens, ksvd_loss

        return encoder_out, encoder_out_lens, ksvd_loss
